// routes/compare.js - COMPLETE FIXED VERSION
...
const validateCourseIds = (courseIds) => {
  console.log('🔍 validateCourseIds received:', courseIds)
  ...
  courseIds.forEach((id) => {
    console.log('🔍 Checking course ID:', id, 'Type:', typeof id)
  ...
  if (invalidIds.length > 0) {
    throw new Error(
      `Invalid course IDs: ${invalidIds.join(', ')}. Course IDs must be valid MongoDB ObjectIds.`
    )
  }
...
router.post('/courses', async (req, res) => {
  try {
    console.log('🔍 POST /api/compare/courses - Body:', req.body)
    ...
    const courses = await Course.find({
      ...
      .select(
        'title description subtitle image price category level language ratings studentsEnrolled totalHours lecturesCount requirements whatYoullLearn isPremium slug features instructor'
      )
      .lean()
    ...
    if (
      error.message.includes('Course IDs array is required') ||
...
router.get('/:courseIds', async (req, res) => {
  try {
    ...
    if (
      error.message.includes('Course IDs parameter is required') ||
      error.message.includes('Cannot compare more than 3 courses') ||
      error.message.includes('Invalid course IDs') ||
      error.message.includes('No valid course IDs')
    ) {
...
// @desc    Get available courses for comparison
...
router.get('/available/courses', async (req, res) => {
  try {
    ...
    const courses = await Course.find({
      ...
      .select('_id title price category level ratings studentsEnrolled instructor')
...
// @desc    Add course to comparison
// @route   POST /api/compare/add/:courseId
// @access  Public
router.post('/add/:courseId', async (req, res) => {
  try {
    ...
    res.status(200).json({
      success: true,
      message: 'Course added to comparison',
    })
  } catch (error) {
    console.error('Add to comparison error:', error)
    res.status(500).json({
      success: false,
      message: 'Internal server error while adding course to comparison',
    })
  }
})

// @desc    Remove course from comparison
// @route   DELETE /api/compare/remove/:courseId
// @access  Public
router.delete('/remove/:courseId', async (req, res) => {
  try {
    ...
    res.status(200).json({
      success: true,
      message: 'Course removed from comparison',
    })
  } catch (error) {
    console.error('Remove from comparison error:', error)
    res.status(500).json({
      success: false,
      message: 'Internal server error while removing course from comparison',
    })
  }
})
...
router.delete('/clear', async (req, res) => {
  try {
    ...
    res.status(500).json({
      success: false,
      message: 'Internal server error while clearing comparison',
    })
  }
})
